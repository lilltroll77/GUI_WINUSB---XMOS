...
QcBackgroundItem *QcGaugeWidget::addBackground(float position , int startA , int stopA )
{
    QcBackgroundItem * item = new QcBackgroundItem(this);
    item->setPosition(position);
    item->setStartAngle(startA);
    item->setStopAngle(stopA);
    mItems.append(item);
    return item;
}
...
int QcItem::StartAngle(){
    return mStartAngle;
}

void QcItem::setStartAngle(int angle){
   mStartAngle = (angle);
}

int QcItem::StopAngle(){
    return mStopAngle;
}

void QcItem::setStopAngle(int angle){
   mStopAngle = (angle);
}
...
void QcBackgroundItem::draw(QPainter* painter)
{
    ...
    painter->drawPie(adjustRect(position()),16*StartAngle(),16*(StopAngle()-StartAngle()));
}
